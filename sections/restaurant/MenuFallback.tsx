...
export default function MenuFallback({ dict, locale }: { dict: Dictionary; locale: Locale }) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-5 py-20 md:px-8">
      ...
      <div className="mb-12 flex flex-col items-center gap-3 rounded-3xl bg-white p-8 text-center shadow-xl shadow-deep/5 ring-1 ring-deep/5">
        <a href={restaurantConfig.menuPdf} download className="btn-primary">
          📄 {dict.menuSection.download}
        </a>
        <p className="text-sm text-deep/50">{dict.menuSection.downloadNote}</p>
      </div>
        ...
        {[{ id: "tutte", label: "•••" }, ...categories].map((cat) => (
          <button
            ...
            className={`relative rounded-full px-5 py-2.5 text-sm font-bold transition-colors ${
              active === cat.id ? "text-white" : "bg-sand-light text-deep hover:bg-sand"
            }`}
          >
            {active === cat.id && (
              <motion.span
          ...
          {visible.map((cat) => (
            ...
            >
              <div className="mb-6 flex items-baseline gap-4">
                ...
                <span className="h-px flex-1 bg-deep/10" aria-hidden="true" />
              </div>
              ...
              <ul className="grid gap-4 md:grid-cols-2">
                {cat.items.map((item) => (
                  <li
                    key={item.name}
                    className="group rounded-3xl bg-white p-6 shadow-lg shadow-deep/5 ring-1 ring-deep/5 transition-transform duration-300 hover:-translate-y-1"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <h3 className="font-display text-lg font-semibold">{item.name}</h3>
                      <span className="shrink-0 rounded-full bg-sand-light px-3 py-1 text-sm font-bold tabular-nums text-sea">
                        {item.price}
                      </span>
                    </div>
                    <p className="mt-2 text-sm leading-relaxed text-deep/70">{item.desc}</p>
                  </li>
                ))}
              </ul>
            </motion.section>
      ...
      <div className="mt-16 rounded-3xl bg-sand-light p-6 text-center font-medium text-deep/80">
        <p>{dict.menuPage.note}</p>
        <Link
          href={`/${locale}/allergeni`}
          className="group mt-3 inline-flex items-center gap-2 font-bold text-coral"
        >
